...
**Implemented:** All 20 stages (as of Feb 2026)

Worker stages 1–20 exist under `services/worker/src/workflow_engine/stages/`. Chat and AI streaming routes are mounted in the orchestrator; agent/RAG endpoints are available at `/agents/*` and `/agents/rag/search`, `/agents/rag/index` when the worker and Chroma are configured.
...
### 3.1 Route Registration (as of Feb 2026)

Routes are registered in **`services/orchestrator/routes.ts`** (not `index.ts`). The following are now mounted:

- **`ai-feedback.ts`** – Mounted at `/api/ai/feedback` (includes `POST /api/ai/feedback/rag/rebuild` for feedback→RAG loop).
- **`ai-streaming.ts`** – Mounted at `/api/ai` (SSE at `/api/ai/stream`, health at `/api/ai/stream/health`).
- **`chat.routes.ts`** – Mounted at `/api/chat` (requireAuth) for artifact-scoped chat.

Other route files listed in earlier gap audits may also be mounted; see `routes.ts` for the current set.
...
### Phase 1: Critical API Integration (DONE as of Feb 2026)

**Route registration is in `services/orchestrator/routes.ts`.** Chat and AI streaming are mounted:

- `app.use("/api/chat", requireAuth, chatRouter)` – artifact-scoped chat (from `chat.routes.ts`).
- `app.use("/api/ai", aiStreamingRouter)` – SSE at `/api/ai/stream`, health at `/api/ai/stream/health`.
- `app.use("/api/ai/feedback", aiFeedbackRouter)` – includes `POST /api/ai/feedback/rag/rebuild` (STEWARD) for feedback→RAG.
