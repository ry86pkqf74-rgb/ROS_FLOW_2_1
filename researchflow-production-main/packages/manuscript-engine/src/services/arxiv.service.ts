...
export class ArxivService {
   ...
   * Accepts either a string (direct call) or a body object (bridge: { arxivId: string }).
   */
  async getArticle(arxivIdOrParams: string | { arxivId?: string }): Promise<ArxivArticle | null> {
    const arxivId =
      typeof arxivIdOrParams === 'string'
        ? arxivIdOrParams
        : (arxivIdOrParams?.arxivId ?? '').trim();
