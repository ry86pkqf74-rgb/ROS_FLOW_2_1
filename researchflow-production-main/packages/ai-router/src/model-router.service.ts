...
export class ModelRouterService {
  ...
  private qwenLocal: QwenLocalProvider | null = null;
  private mercury: MercuryCoderProvider | null = null;
  ...
  constructor(config: Partial<AIRouterConfig> = {}) {
    ...
    if (this.config.mercuryApiKey && process.env.MERCURY_ENABLED !== 'false') {
      this.mercury = new MercuryCoderProvider({
        apiKey: this.config.mercuryApiKey,
      });
    }

    // Initialize Qwen Local provider if enabled
    if (process.env.LOCAL_MODEL_ENABLED === 'true') {
      this.qwenLocal = new QwenLocalProvider({
        endpoint: process.env.LOCAL_MODEL_ENDPOINT,
        model: process.env.LOCAL_MODEL_NAME,
      });
      // Initialize async - don't block constructor
      this.qwenLocal.initialize().catch((err) => {
        console.warn('[ModelRouter] Qwen Local initialization failed:', err.message);
      });
    }
  ...
  /**
   * Check if LOCAL tier should be used for this request
   */
  private async shouldUseLocalTier(request: AIRouterRequest, tier: ModelTier): Promise<boolean> {
    // Only consider LOCAL if tier is LOCAL or no tier forced
    if (tier !== 'LOCAL') {
      return false;
    }

    // Check if LOCAL provider is available
    if (!this.qwenLocal || !this.qwenLocal.isAvailable()) {
      return false;
    }

    // Check if task is blocked from LOCAL (PHI compliance)
    if (LOCAL_BLOCKED_TASKS.has(request.taskType)) {
      return false;
    }

    // Check if task should use LOCAL
    return shouldUseLocal(request.taskType, {
      requiresAudit: false, // Add audit checking logic if needed
      preferLocal: true,
  ...
  /**
   * Invoke the model for a specific tier with provider fallback
   *
   * Provider fallback order: Primary → OpenAI → Anthropic → Ollama (local)
   * This ensures high availability even when individual providers have outages.
   */
  private async invokeModel(
    ...
    if (config.provider === 'local') {
      ...
      if (await this.shouldUseLocalTier(request, tier)) {
        try {
          return await this.invokeQwenLocal(request, config, tier);
        } catch (err) {
          errors.push(`LOCAL: ${err instanceof Error ? err.message : String(err)}`);
        }
      }
      // Fall back to NANO if LOCAL not available
      const nanoConfig = MODEL_CONFIGS['NANO'];
      return this.invokeAnthropic(request, nanoConfig, 'NANO');
    }

    // Provider fallback chain based on primary provider
    const fallbackOrder: Array<'mercury' | 'openai' | 'anthropic' | 'local'> =
      config.provider === 'mercury'
        ? ['mercury', 'openai', 'anthropic', 'local']
    ...
    for (const provider of fallbackOrder) {
      try {
        if (provider === 'anthropic' && this.anthropic) {
          return await this.invokeAnthropic(request, config, tier);
        } else if (provider === 'mercury' && this.mercury) {
          return await this.invokeMercury(request, config, tier);
        } else if (provider === 'openai' && this.openai) {
          return await this.invokeOpenAI(request, config, tier);
        } else if (provider === 'local' && this.qwenLocal?.isAvailable()) {
          // Fallback to local - use LOCAL tier config
          const localConfig = MODEL_CONFIGS['LOCAL'];
          return await this.invokeQwenLocal(request, localConfig, 'LOCAL');
        }
      } catch (err) {
        const errMsg = err instanceof Error ? err.message : String(err);
        errors.push(`${provider}: ${errMsg}`);
        console.warn(`[ModelRouter] Provider ${provider} failed, trying next: ${errMsg}`);
        continue; // Try next provider in fallback chain
      }
    }

    // All providers failed
    throw new Error(`All providers failed for tier ${tier}: ${errors.join('; ')}`);
  }

  /**
   * Invoke Qwen Local via Ollama
   */
  private async invokeQwenLocal(
    request: AIRouterRequest,
    ...
    const response = await this.qwenLocal.complete({
      messages: [
    ...
    return {
      ...
      usage: {
        ...
        estimatedCostUsd: 0, // Free - local inference
      },
      qualityGate: {
        passed: true,
        checks: [],
  ...
  /**
   * Invoke Mercury (Inception Labs) API
   */
  private async invokeMercury(
    request: AIRouterRequest,
    config: typeof MODEL_CONFIGS[ModelTier],
    tier: ModelTier
  ): Promise<AIRouterResponse> {
    if (!this.mercury) {
      throw new Error('Mercury client not initialized');
    }
    ...
    const response = await this.mercury.chat(messages, {
      model: config.model as 'mercury' | 'mercury-coder' | 'mercury-coder-small',
      temperature: request.temperature ?? config.temperature,
      max_tokens: request.maxTokens || config.maxTokens,
      taskType: request.taskType,
      tier,
      ...
      phaseId: request.metadata?.stageId,
    });

    const content = response.content;
    const inputTokens = response.usage.inputTokens;
    const outputTokens = response.usage.outputTokens;
    const estimatedCostUsd = this.calculateCost(tier, inputTokens, outputTokens);
