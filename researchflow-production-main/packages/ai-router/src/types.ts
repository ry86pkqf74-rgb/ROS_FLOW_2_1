 ...
 */
export type AIProvider = 'anthropic' | 'openai' | 'together' | 'local' | 'mercury';
...
const MERCURY_COST_PER_M_TOKEN = { input: 0.25, output: 1.0 };
const MERCURY_CHAT_MODEL =
  process.env.MERCURY_CHAT_MODEL ||
  process.env.INCEPTION_CHAT_MODEL ||
  process.env.MERCURY_MODEL ||
  process.env.INCEPTION_MODEL ||
  'mercury';
const MERCURY_KEY =
  process.env.MERCURY_API_KEY ||
  process.env.INCEPTION_API_KEY ||
  process.env.INCEPTIONLABS_API_KEY;
const MERCURY_ENABLED = !!MERCURY_KEY && process.env.MERCURY_ENABLED !== 'false';

const PROVIDER_VALUES = new Set<AIProvider>([
  'anthropic',
  'openai',
  'together',
  'local',
...
function parseProvider(value: string | undefined): AIProvider | undefined {
  ...
  return PROVIDER_VALUES.has(normalized as AIProvider) ? (normalized as AIProvider) : undefined;
}

function getProviderOverride(tier: ModelTier): AIProvider | undefined {
  return parseProvider(process.env[`AI_TIER_PROVIDER_${tier}`]);
}

function getModelOverride(tier: ModelTier): string | undefined {
  return process.env[`AI_TIER_MODEL_${tier}`];
}

function resolveProvider(tier: ModelTier, fallback: AIProvider): AIProvider {
  return getProviderOverride(tier) ?? fallback;
}

function resolveModel(tier: ModelTier, provider: AIProvider, fallback: string): string {
  const override = getModelOverride(tier);
  if (override) return override;
  if (provider === 'mercury') return MERCURY_CHAT_MODEL;
  return fallback;
}
...
function resolveCost(
  ...
  return provider === 'mercury' ? MERCURY_COST_PER_M_TOKEN : fallback;
}
...
export const MODEL_CONFIGS: Record<ModelTier, ModelConfig> = {
  LOCAL: {
    ...
    maxTokens: 8192,
    temperature: 0.1,
    costPerMToken: { input: 0, output: 0 }, // Free - local inference
  },
  NANO: {
    provider: NANO_PROVIDER,
    model: resolveModel('NANO', NANO_PROVIDER, 'claude-haiku-4-5-20251001'),
    maxTokens: 1024,
    temperature: 0.1,
    costPerMToken: resolveCost(NANO_PROVIDER, { input: 0.25, output: 1.25 }),
  },
  MINI: {
    provider: MINI_PROVIDER,
    model: resolveModel('MINI', MINI_PROVIDER, 'claude-sonnet-4-5-20250929'),
    maxTokens: 4096,
    temperature: 0.3,
  ...
  FRONTIER: {
    ...
    model: resolveModel('FRONTIER', FRONTIER_PROVIDER, 'claude-opus-4-5-20251101'),
    maxTokens: 8192,
    temperature: 0.5,
 ...
 */
export const TIER_ESCALATION_ORDER: ModelTier[] = ['LOCAL', 'NANO', 'MINI', 'FRONTIER'];
