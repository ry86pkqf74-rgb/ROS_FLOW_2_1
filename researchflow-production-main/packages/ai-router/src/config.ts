...
export interface ProviderConfig {
  ...
  /** Mercury / Inception Labs API key */
...
export function loadConfig(): AIRouterFullConfig {
  ...
  return {
    ...
    provider: {
      ...
      mercuryApiKey:
        env.MERCURY_API_KEY || env.INCEPTION_API_KEY || env.INCEPTIONLABS_API_KEY,
...
export function validateConfig(config: AIRouterFullConfig): { valid: boolean; errors: string[] } {
  ...
  if (
    ...
    !config.provider.togetherApiKey &&
    !config.provider.mercuryApiKey
  ) {
...
export function getConfigSummary(): Record<string, unknown> {
  ...
  return {
    ...
    provider: {
      ...
      hasMercuryKey: !!config.provider.mercuryApiKey,
