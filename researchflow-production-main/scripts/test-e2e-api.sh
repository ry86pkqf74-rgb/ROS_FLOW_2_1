...
# Worker runs on internal Docker network only - test via orchestrator proxy or skip
WORKER_HEALTH=$(curl -s --max-time 5 $WORKER_URL/health 2>/dev/null || echo '{"error":"not exposed"}')
if echo "$WORKER_HEALTH" | grep -qE '"status":\s*"(ok|healthy)"'; then
    pass "Worker healthy"
elif echo "$WORKER_HEALTH" | grep -q "not exposed"; then
    info "Worker port not exposed to host (internal service - this is expected)"
    # Test worker through orchestrator's internal connectivity
    WORKER_VIA_ORCH=$(curl -s $BASE_URL/api/worker/health 2>/dev/null || echo '{}')
    if [ -n "$WORKER_VIA_ORCH" ] && [ "$WORKER_VIA_ORCH" != "{}" ]; then
        pass "Worker accessible via orchestrator"
    else
        info "Worker internal connectivity - check docker-compose logs worker"
    fi
else
...
GUIDELINE_HEALTH=$(curl -s $GUIDELINE_URL/health 2>/dev/null || echo '{"error":"unavailable"}')
if echo "$GUIDELINE_HEALTH" | grep -qE '"status":\s*"(ok|healthy)"'; then
    pass "Guideline engine healthy"
else
    info "Guideline engine not available (optional service)"
fi
echo ""
