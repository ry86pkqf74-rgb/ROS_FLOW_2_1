# agent-policy-review - Wiring Documentation

**Agent Key:** `agent-policy-review`  
...
Policy and guideline compliance review

## Deployment Configuration

### Docker Compose Service

**Service Name:** `agent-policy-review`  
**Container Name:** `researchflow-agent-policy-review`  
**Internal Port:** 8000  
...
```bash
docker compose exec agent-policy-review curl -f http://localhost:8000/health
```
...
```bash
# Test all agents including agent-policy-review
...
- Writes artifact to `/data/artifacts/validation/agent-policy-review/<timestamp>/summary.json`
...
grep -A 20 "agent-policy-review:" docker-compose.yml

# 2. Verify AGENT_ENDPOINTS_JSON includes this agent
docker compose config | grep AGENT_ENDPOINTS_JSON | grep "agent-policy-review"
...
```bash
# Check logs
docker compose logs --tail=50 agent-policy-review
...
docker compose exec agent-policy-review curl -v http://localhost:8000/health

# Check environment
docker compose exec agent-policy-review env | grep -E 'AI_BRIDGE|ORCHESTRATOR'

# Restart service
docker compose restart agent-policy-review
```

### Routing failures
```bash
# Verify orchestrator has agent in registry
docker compose exec orchestrator sh -c 'echo $AGENT_ENDPOINTS_JSON' | python3 -m json.tool | grep "agent-policy-review"

# Check router registration
grep "agent-policy-review" services/orchestrator/src/routes/ai-router.ts
...
**Path:** `/data/artifacts/agent-policy-review/`
...
- **Service Definition:** `docker-compose.yml` (search for `agent-policy-review:`)
- **Router Registration:** `services/orchestrator/src/routes/ai-router.ts`
- **Agent Implementation:** `services/agents/agent-policy-review/`
