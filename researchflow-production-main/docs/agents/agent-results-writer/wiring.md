# agent-results-writer - Wiring Documentation

**Agent Key:** `agent-results-writer`  
...
**Service Name:** `agent-results-writer`  
**Container Name:** `researchflow-agent-results-writer`  
**Internal Port:** 8000  
...
```bash
docker compose exec agent-results-writer curl -f http://localhost:8000/health
```
...
```bash
# Test all agents including agent-results-writer
...
- Writes artifact to `/data/artifacts/validation/agent-results-writer/<timestamp>/summary.json`
...
grep -A 20 "agent-results-writer:" docker-compose.yml

# 2. Verify AGENT_ENDPOINTS_JSON includes this agent
docker compose config | grep AGENT_ENDPOINTS_JSON | grep "agent-results-writer"

# 3. Build and start
docker compose build agent-results-writer
...
```bash
# Check logs
docker compose logs --tail=50 agent-results-writer
...
docker compose exec agent-results-writer curl -v http://localhost:8000/health

# Check environment
docker compose exec agent-results-writer env | grep -E 'AI_BRIDGE|ORCHESTRATOR'

# Restart service
docker compose restart agent-results-writer
```

### Routing failures
```bash
# Verify orchestrator has agent in registry
docker compose exec orchestrator sh -c 'echo $AGENT_ENDPOINTS_JSON' | python3 -m json.tool | grep "agent-results-writer"

# Check router registration
grep "agent-results-writer" services/orchestrator/src/routes/ai-router.ts
...
curl -X POST http://127.0.0.1:3001/api/ai/router/dispatch \
  ...
  -d '{
    "task_type": "SECTION_WRITE_RESULTS",
    "request_id": "test-001",
    "mode": "DEMO",
    "inputs": {}
  }'
...
**Path:** `/data/artifacts/agent-results-writer/`
...
- **Service Definition:** `docker-compose.yml` (search for `agent-results-writer:`)
- **Router Registration:** `services/orchestrator/src/routes/ai-router.ts`
- **Agent Implementation:** `services/agents/agent-results-writer/`
