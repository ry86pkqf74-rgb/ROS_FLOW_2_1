# agent-intro-writer - Wiring Documentation

**Agent Key:** `agent-intro-writer`  
...
**Service Name:** `agent-intro-writer`  
**Container Name:** `researchflow-agent-intro-writer`  
**Internal Port:** 8000  
...
```bash
docker compose exec agent-intro-writer curl -f http://localhost:8000/health
```
...
```bash
# Test all agents including agent-intro-writer
...
- Writes artifact to `/data/artifacts/validation/agent-intro-writer/<timestamp>/summary.json`
...
grep -A 20 "agent-intro-writer:" docker-compose.yml

# 2. Verify AGENT_ENDPOINTS_JSON includes this agent
docker compose config | grep AGENT_ENDPOINTS_JSON | grep "agent-intro-writer"

# 3. Build and start
docker compose build agent-intro-writer
docker compose up -d agent-intro-writer
...
```bash
# Check logs
docker compose logs --tail=50 agent-intro-writer
...
docker compose exec agent-intro-writer curl -v http://localhost:8000/health

# Check environment
docker compose exec agent-intro-writer env | grep -E 'AI_BRIDGE|ORCHESTRATOR'

# Restart service
docker compose restart agent-intro-writer
```

### Routing failures
```bash
# Verify orchestrator has agent in registry
docker compose exec orchestrator sh -c 'echo $AGENT_ENDPOINTS_JSON' | python3 -m json.tool | grep "agent-intro-writer"

# Check router registration
grep "agent-intro-writer" services/orchestrator/src/routes/ai-router.ts
...
**Path:** `/data/artifacts/agent-intro-writer/`
...
- **Service Definition:** `docker-compose.yml` (search for `agent-intro-writer:`)
- **Router Registration:** `services/orchestrator/src/routes/ai-router.ts`
- **Agent Implementation:** `services/agents/agent-intro-writer/`
