# agent-stage2-synthesize - Wiring Documentation

**Agent Key:** `agent-stage2-synthesize`  
...
Evidence synthesis (legacy stub)

## Deployment Configuration

### Docker Compose Service

**Service Name:** `agent-stage2-synthesize`  
**Container Name:** `researchflow-agent-stage2-synthesize`  
**Internal Port:** 8000  
...
```bash
docker compose exec agent-stage2-synthesize curl -f http://localhost:8000/health
```
...
```bash
# Test all agents including agent-stage2-synthesize
...
- Writes artifact to `/data/artifacts/validation/agent-stage2-synthesize/<timestamp>/summary.json`
...
grep -A 20 "agent-stage2-synthesize:" docker-compose.yml

# 2. Verify AGENT_ENDPOINTS_JSON includes this agent
docker compose config | grep AGENT_ENDPOINTS_JSON | grep "agent-stage2-synthesize"

# 3. Build and start
docker compose build agent-stage2-synthesize
...
```bash
# Check logs
docker compose logs --tail=50 agent-stage2-synthesize
...
docker compose exec agent-stage2-synthesize curl -v http://localhost:8000/health

# Check environment
docker compose exec agent-stage2-synthesize env | grep -E 'AI_BRIDGE|ORCHESTRATOR'

# Restart service
docker compose restart agent-stage2-synthesize
```

### Routing failures
```bash
# Verify orchestrator has agent in registry
docker compose exec orchestrator sh -c 'echo $AGENT_ENDPOINTS_JSON' | python3 -m json.tool | grep "agent-stage2-synthesize"

# Check router registration
grep "agent-stage2-synthesize" services/orchestrator/src/routes/ai-router.ts
...
**Path:** `/data/artifacts/agent-stage2-synthesize/`
...
- **Service Definition:** `docker-compose.yml` (search for `agent-stage2-synthesize:`)
- **Router Registration:** `services/orchestrator/src/routes/ai-router.ts`
- **Agent Implementation:** `services/agents/agent-stage2-synthesize/`
