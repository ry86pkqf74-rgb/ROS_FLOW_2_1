# agent-rag-ingest - Wiring Documentation

**Agent Key:** `agent-rag-ingest`  
...
Document chunking and embedding to ChromaDB

## Deployment Configuration

### Docker Compose Service

**Service Name:** `agent-rag-ingest`  
**Container Name:** `researchflow-agent-rag-ingest`  
**Internal Port:** 8000  
...
```bash
docker compose exec agent-rag-ingest curl -f http://localhost:8000/health
```
...
```bash
# Test all agents including agent-rag-ingest
...
- Writes artifact to `/data/artifacts/validation/agent-rag-ingest/<timestamp>/summary.json`
...
grep -A 20 "agent-rag-ingest:" docker-compose.yml

# 2. Verify AGENT_ENDPOINTS_JSON includes this agent
docker compose config | grep AGENT_ENDPOINTS_JSON | grep "agent-rag-ingest"

# 3. Build and start
docker compose build agent-rag-ingest
docker compose up -d agent-rag-ingest
...
```bash
# Check logs
docker compose logs --tail=50 agent-rag-ingest
...
docker compose exec agent-rag-ingest curl -v http://localhost:8000/health

# Check environment
docker compose exec agent-rag-ingest env | grep -E 'AI_BRIDGE|ORCHESTRATOR'

# Restart service
docker compose restart agent-rag-ingest
```

### Routing failures
```bash
# Verify orchestrator has agent in registry
docker compose exec orchestrator sh -c 'echo $AGENT_ENDPOINTS_JSON' | python3 -m json.tool | grep "agent-rag-ingest"

# Check router registration
grep "agent-rag-ingest" services/orchestrator/src/routes/ai-router.ts
...
**Path:** `/data/artifacts/agent-rag-ingest/`
...
- **Service Definition:** `docker-compose.yml` (search for `agent-rag-ingest:`)
- **Router Registration:** `services/orchestrator/src/routes/ai-router.ts`
- **Agent Implementation:** `services/agents/agent-rag-ingest/`
