# agent-discussion-writer - Wiring Documentation

**Agent Key:** `agent-discussion-writer`  
...
**Service Name:** `agent-discussion-writer`  
**Container Name:** `researchflow-agent-discussion-writer`  
**Internal Port:** 8000  
...
```bash
docker compose exec agent-discussion-writer curl -f http://localhost:8000/health
```
...
```bash
# Test all agents including agent-discussion-writer
...
- Writes artifact to `/data/artifacts/validation/agent-discussion-writer/<timestamp>/summary.json`
...
grep -A 20 "agent-discussion-writer:" docker-compose.yml

# 2. Verify AGENT_ENDPOINTS_JSON includes this agent
docker compose config | grep AGENT_ENDPOINTS_JSON | grep "agent-discussion-writer"

# 3. Build and start
docker compose build agent-discussion-writer
...
```bash
# Check logs
docker compose logs --tail=50 agent-discussion-writer
...
docker compose exec agent-discussion-writer curl -v http://localhost:8000/health

# Check environment
docker compose exec agent-discussion-writer env | grep -E 'AI_BRIDGE|ORCHESTRATOR'

# Restart service
docker compose restart agent-discussion-writer
```

### Routing failures
```bash
# Verify orchestrator has agent in registry
docker compose exec orchestrator sh -c 'echo $AGENT_ENDPOINTS_JSON' | python3 -m json.tool | grep "agent-discussion-writer"

# Check router registration
grep "agent-discussion-writer" services/orchestrator/src/routes/ai-router.ts
...
curl -X POST http://127.0.0.1:3001/api/ai/router/dispatch \
  ...
  -d '{
    "task_type": "SECTION_WRITE_DISCUSSION",
    "request_id": "test-001",
    "mode": "DEMO",
    "inputs": {}
  }'
...
**Path:** `/data/artifacts/agent-discussion-writer/`
...
- **Service Definition:** `docker-compose.yml` (search for `agent-discussion-writer:`)
- **Router Registration:** `services/orchestrator/src/routes/ai-router.ts`
- **Agent Implementation:** `services/agents/agent-discussion-writer/`
