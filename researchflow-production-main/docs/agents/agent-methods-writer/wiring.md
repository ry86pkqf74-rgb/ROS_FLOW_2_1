# agent-methods-writer - Wiring Documentation

**Agent Key:** `agent-methods-writer`  
...
**Service Name:** `agent-methods-writer`  
**Container Name:** `researchflow-agent-methods-writer`  
**Internal Port:** 8000  
...
```bash
docker compose exec agent-methods-writer curl -f http://localhost:8000/health
```
...
```bash
# Test all agents including agent-methods-writer
...
- Writes artifact to `/data/artifacts/validation/agent-methods-writer/<timestamp>/summary.json`
...
grep -A 20 "agent-methods-writer:" docker-compose.yml

# 2. Verify AGENT_ENDPOINTS_JSON includes this agent
docker compose config | grep AGENT_ENDPOINTS_JSON | grep "agent-methods-writer"

# 3. Build and start
docker compose build agent-methods-writer
docker compose up -d agent-methods-writer
...
```bash
# Check logs
docker compose logs --tail=50 agent-methods-writer
...
docker compose exec agent-methods-writer curl -v http://localhost:8000/health

# Check environment
docker compose exec agent-methods-writer env | grep -E 'AI_BRIDGE|ORCHESTRATOR'

# Restart service
docker compose restart agent-methods-writer
```

### Routing failures
```bash
# Verify orchestrator has agent in registry
docker compose exec orchestrator sh -c 'echo $AGENT_ENDPOINTS_JSON' | python3 -m json.tool | grep "agent-methods-writer"

# Check router registration
grep "agent-methods-writer" services/orchestrator/src/routes/ai-router.ts
...
curl -X POST http://127.0.0.1:3001/api/ai/router/dispatch \
  ...
  -d '{
    "task_type": "SECTION_WRITE_METHODS",
    "request_id": "test-001",
    "mode": "DEMO",
    "inputs": {}
  }'
...
**Path:** `/data/artifacts/agent-methods-writer/`
...
- **Service Definition:** `docker-compose.yml` (search for `agent-methods-writer:`)
- **Router Registration:** `services/orchestrator/src/routes/ai-router.ts`
- **Agent Implementation:** `services/agents/agent-methods-writer/`
