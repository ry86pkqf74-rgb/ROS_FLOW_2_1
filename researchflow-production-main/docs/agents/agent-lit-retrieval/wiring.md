# agent-lit-retrieval - Wiring Documentation

**Agent Key:** `agent-lit-retrieval`  
...
Deterministic PubMed search

## Deployment Configuration

### Docker Compose Service

**Service Name:** `agent-lit-retrieval`  
**Container Name:** `researchflow-agent-lit-retrieval`  
**Internal Port:** 8000  
...
```bash
docker compose exec agent-lit-retrieval curl -f http://localhost:8000/health
```
...
```bash
# Test all agents including agent-lit-retrieval
...
- Writes artifact to `/data/artifacts/validation/agent-lit-retrieval/<timestamp>/summary.json`
...
grep -A 20 "agent-lit-retrieval:" docker-compose.yml

# 2. Verify AGENT_ENDPOINTS_JSON includes this agent
docker compose config | grep AGENT_ENDPOINTS_JSON | grep "agent-lit-retrieval"

# 3. Build and start
docker compose build agent-lit-retrieval
docker compose up -d agent-lit-retrieval
...
docker compose exec agent-lit-retrieval curl -v http://localhost:8000/health

# Check environment
docker compose exec agent-lit-retrieval env | grep -E 'AI_BRIDGE|ORCHESTRATOR'
...
docker compose exec orchestrator sh -c 'echo $AGENT_ENDPOINTS_JSON' | python3 -m json.tool | grep "agent-lit-retrieval"

# Check router registration
grep "agent-lit-retrieval" services/orchestrator/src/routes/ai-router.ts
...
**Path:** `/data/artifacts/agent-lit-retrieval/`
...
- **Service Definition:** `docker-compose.yml` (search for `agent-lit-retrieval:`)
- **Router Registration:** `services/orchestrator/src/routes/ai-router.ts`
- **Agent Implementation:** `services/agents/agent-lit-retrieval/`
