# agent-rag-retrieve - Wiring Documentation

**Agent Key:** `agent-rag-retrieve`  
...
Vector similarity search and grounding

## Deployment Configuration

### Docker Compose Service

**Service Name:** `agent-rag-retrieve`  
**Container Name:** `researchflow-agent-rag-retrieve`  
**Internal Port:** 8000  
...
```bash
docker compose exec agent-rag-retrieve curl -f http://localhost:8000/health
```
...
```bash
# Test all agents including agent-rag-retrieve
...
- Writes artifact to `/data/artifacts/validation/agent-rag-retrieve/<timestamp>/summary.json`
...
grep -A 20 "agent-rag-retrieve:" docker-compose.yml

# 2. Verify AGENT_ENDPOINTS_JSON includes this agent
docker compose config | grep AGENT_ENDPOINTS_JSON | grep "agent-rag-retrieve"

# 3. Build and start
docker compose build agent-rag-retrieve
docker compose up -d agent-rag-retrieve
...
```bash
# Check logs
docker compose logs --tail=50 agent-rag-retrieve
...
docker compose exec agent-rag-retrieve curl -v http://localhost:8000/health

# Check environment
docker compose exec agent-rag-retrieve env | grep -E 'AI_BRIDGE|ORCHESTRATOR'

# Restart service
docker compose restart agent-rag-retrieve
```

### Routing failures
```bash
# Verify orchestrator has agent in registry
docker compose exec orchestrator sh -c 'echo $AGENT_ENDPOINTS_JSON' | python3 -m json.tool | grep "agent-rag-retrieve"

# Check router registration
grep "agent-rag-retrieve" services/orchestrator/src/routes/ai-router.ts
...
**Path:** `/data/artifacts/agent-rag-retrieve/`
...
- **Service Definition:** `docker-compose.yml` (search for `agent-rag-retrieve:`)
- **Router Registration:** `services/orchestrator/src/routes/ai-router.ts`
- **Agent Implementation:** `services/agents/agent-rag-retrieve/`
