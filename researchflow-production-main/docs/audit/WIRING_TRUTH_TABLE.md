...
_Update (2026-01-30): docker-compose.yml now includes the variables below. Keep
.env populated with real values for integrations and production runs._
...
_Update (2026-01-30): docker-compose.yml now exports both `ARTIFACT_PATH` and
`ARTIFACTS_PATH` for backward compatibility. Documentation should standardize
on `ARTIFACTS_PATH` going forward._
