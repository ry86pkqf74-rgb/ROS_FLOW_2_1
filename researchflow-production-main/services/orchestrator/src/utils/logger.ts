...
/**
 * Normalize correlation ID: prefer explicit correlationId, fallback to requestId
 */
function normalizeCorrelationId(ctx?: LogContext): string | undefined {
  if (!ctx) return undefined;
  const cid = (ctx.correlationId as string | undefined) || (ctx.requestId as string | undefined);
  return cid;
}
