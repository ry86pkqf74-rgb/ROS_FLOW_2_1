...
/**
 * Get client IP for key generation
 */
 ...
 * Applied only when req.user is set; skip when unauthenticated.
 */
export async function createPerUserLimiter() {
  const client = await initializeRedisClient();
  ...
  return rateLimit({
    ...
    handler: (req: Request, res: Response) => {
      logger.warn('Per-user rate limit exceeded', {
        key: req.user && typeof req.user === 'object' && 'id' in req.user ? `user:${(req.user as any).id}` : 'n/a',
        path: req.path,
        method: req.method,
      });
      res.status(429).json({
...
/**
 * Per-IP rate limiter for unauthenticated requests (20 requests per minute).
 * Applied only when req.user is not set; skip when authenticated.
 */
export async function createPerIpLimiter() {
  const client = await initializeRedisClient();
