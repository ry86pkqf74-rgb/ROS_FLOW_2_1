...
export function ethicsGate(options: EthicsGateOptions) {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      // For low risk, require explicit confirmation ONLY if requireConfirmation is enabled
      ...
      // For low risk: auto-approve if either confirmation received OR confirmation not required
      // FIX: Previously required confirmed=true even when requireConfirmation=false
      if (riskLevel === 'LOW' && (confirmed || !requireConfirmation)) {
        await logAction({
          eventType: 'ETHICS_GATE',
          ...
          resourceType: 'ethics_approval',
          resourceId: approvalId,
          details: { taskType, riskLevel, confirmationWaived: !requireConfirmation }
        });
