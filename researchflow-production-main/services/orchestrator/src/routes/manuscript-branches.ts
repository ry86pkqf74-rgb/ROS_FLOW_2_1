...
const createManuscriptBranchSchema = z.object({
  name: z.string().min(1).max(100),
  fromVersionId: z.string().min(1),
});

const mergeManuscriptBranchSchema = z.object({
  targetBranch: z.string().min(1).max(100),
  strategy: z.enum(["ours", "theirs", "manual"]).default("manual"),
});
...
manuscriptBranchingRoutes.post(
  "/:id/branches",
  requireRole("RESEARCHER"),
  async (req: Request, res: Response) => {
    try {
      ...
      const branch = await manuscriptVersionService.createBranch(
        id,
        parsed.data.name,
      ...
      console.error("[manuscript-branches] Create branch error:", error);
      res.status(500).json({ error: "Failed to create branch" });
    }
  }
);
...
manuscriptBranchingRoutes.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const result = await manuscriptVersionService.mergeBranch(
        branchId,
      ...
      console.error("[manuscript-branches] Merge branch error:", error);
      res.status(500).json({ error: "Failed to merge branch" });
    }
  }
);

manuscriptBranchingRoutes.get(
  "/:id/branches/:branchId/diff",
  requireRole("VIEWER"),
  async (req: Request, res: Response) => {
    try {
      const { branchId } = req.params;
      ...
      let to = typeof toVersionId === "string" ? toVersionId : undefined;

      if (!from || !to) {
        const base = await manuscriptVersionService.getBranchBaseVersionId(branchId);
        const head = await manuscriptVersionService.getBranchHeadRevision(branchId);
        from = from || base || head?.id;
        to = to || head?.id;
      }

      if (!from || !to) {
        return res.status(404).json({ error: "Branch versions not found" });
      }

      const diff = await manuscriptVersionService.diffVersions(from, to);
      res.json(diff);
    } catch (error: any) {
      console.error("[manuscript-branches] Diff error:", error);
...
manuscriptBranchingRoutes.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res.status(500).json({ error: "Failed to run refinement" });
    }
  }
);
...
manuscriptBranchingRoutes.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      console.error("[manuscript-branches] Version history error:", error);
