...
/**
 * POST /api/ai/feedback/rag/rebuild
 * Rebuild RAG collection ai_feedback_guidance from aggregated feedback (STEWARD or ADMIN).
 */
router.post(
  '/rag/rebuild',
  requireRole('STEWARD'),
  asyncHandler(async (req: Request, res: Response) => {
    const windowDays = Math.min(
      Math.max(1, parseInt(String(req.query.days), 10) || 90),
      365
    );
    const result = await rebuildFeedbackRAG(windowDays);
    res.json({
      message: 'Feedback RAG rebuild completed',
      ...result,
      windowDays,
    });
  })
