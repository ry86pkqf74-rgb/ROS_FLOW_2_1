...
router.get('/', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    // Query research_projects table (actual schema)
    let query = `
      SELECT p.id, p.research_id, p.title, p.description, p.owner_id,
             p.org_id, p.status, p.governance_mode, p.created_at, p.updated_at
    ...
    const projects = result.rows.map((row: any) => ({
      ...
      workflowCount: 0, // Placeholder - workflows not yet implemented
      settings: {},
      collaborators: [],
    }));
...
router.get('/stats', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const result = await pool.query(
      `SELECT
         COUNT(*) FILTER (WHERE status = 'active' OR status = 'draft') as active_projects,
...
router.post('/', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    // Generate research_id from name (slug format)
    const researchId = `proj-${Date.now()}-${input.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').substring(0, 20)}`;

    const result = await pool!.query(
      `INSERT INTO research_projects (research_id, title, description, owner_id, org_id, status, governance_mode)
       VALUES ($1, $2, $3, $4, $5, 'draft', 'DEMO')
       RETURNING *`,
      [researchId, input.name, input.description || null, userId, input.orgId || null]
    );

    const row = result.rows[0];

    // Note: project_members table doesn't exist in current schema
    // Skipping member insertion for now
