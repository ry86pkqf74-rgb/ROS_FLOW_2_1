...
// Keep in sync with worker PandocExporter.SUPPORTED_FORMATS
const AVAILABLE_EXPORT_FORMATS = ['pdf', 'docx', 'latex', 'html', 'odt', 'epub'] as const;

// Journal templates exposed by API (keys align with worker PandocExporter.JOURNAL_TEMPLATES)
const AVAILABLE_JOURNAL_TEMPLATES = [
  { key: 'jama', name: 'JAMA', description: 'Journal of the American Medical Association (LaTeX template)' },
  { key: 'nejm', name: 'NEJM', description: 'New England Journal of Medicine (LaTeX template)' },
  { key: 'lancet', name: 'The Lancet', description: 'The Lancet (LaTeX template)' },
  { key: 'nature', name: 'Nature', description: 'Nature (LaTeX template)' },
  { key: 'plos', name: 'PLOS', description: 'Public Library of Science (LaTeX template)' },
] as const;
...
const createTemplateSchema = z.object({
  ...
  output_format: z.enum(['docx', 'pdf', 'latex', 'html', 'odt', 'epub']),
...
const exportManuscriptSchema = z.object({
  ...
  output_format: z.enum(['docx', 'pdf', 'latex', 'html', 'odt', 'epub']).default('docx'),
...
router.get('/formats', (_req: Request, res: Response) => {
  res.json({
    ...
    count: AVAILABLE_EXPORT_FORMATS.length,
  });
});
...
router.get('/templates', (_req: Request, res: Response) => {
  res.json({
    templates: AVAILABLE_JOURNAL_TEMPLATES,
    count: AVAILABLE_JOURNAL_TEMPLATES.length,
  });
});
 ...
 *
 * Returns a redacted markdown preview along with the resolved export options.
 * This endpoint does not run Pandoc; it is intended for UI preview and validation.
 */
router.post('/manuscripts/:manuscriptId/preview', async (req: Request, res: Response) => {
  try {
    ...
    let preview = (ms.content || '') as string;
    const patterns = body.redaction_patterns || [];
    for (const pat of patterns) {
      try {
        preview = preview.replace(new RegExp(pat, 'gi'), '[REDACTED]');
      } catch {
        // ignore invalid patterns in preview
      }
    }
    ...
    res.json({
      ...
      csl_style: body.csl_style || null,
      bibliography: body.bibliography || null,
      metadata: body.metadata || {},
      preview_markdown: preview,
    });

  } catch (error) {
    console.error('[export/manuscripts/preview] Error:', error);
...
router.get('/jobs/:jobId/status', async (req: Request, res: Response) => {
  try {
    ...
    const result = await db.execute(sql`
      ...
      FROM export_jobs ej
      LEFT JOIN manuscripts m ON m.id = ej.manuscript_id
      WHERE ej.id = ${jobId} AND ej.user_id = ${userId}
      LIMIT 1
    `);
    ...
    res.json({
      ...
      output_url: result.rows[0].output_url,
      error_code: result.rows[0].error_code,
      error_message: result.rows[0].error_message,
      manuscript_title: result.rows[0].manuscript_title,
    });

  } catch (error) {
    console.error('[export/jobs/status] Error:', error);
    res.status(500).json({ error: 'Failed to get job status' });
  }
});
