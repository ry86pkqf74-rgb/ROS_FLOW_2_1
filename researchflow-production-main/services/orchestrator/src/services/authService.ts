...
// RS256 keys: PEM strings (use \n for newlines in env)
function getPemFromEnv(value: string | undefined): string | null {
...
const JWT_PUBLIC_KEY = (() => {
  ...
  const pem = getPemFromEnv(raw);
  if (process.env.NODE_ENV === 'production') {
    if (!pem || !pem.includes('-----BEGIN')) {
      throw new Error(
        '[SECURITY] FATAL: JWT_PUBLIC_KEY must be set in production with a valid PEM (RS256). ' +
  ...
  if (!pem || !pem.includes('-----BEGIN')) {
    console.warn(
      '[SECURITY] JWT_PUBLIC_KEY not set. Set JWT_PUBLIC_KEY and JWT_PRIVATE_KEY for JWT auth, or use ALLOW_MOCK_AUTH=true in development.'
    );
    return null;
  }
  return pem;
...
const JWT_PRIVATE_KEY = (() => {
  ...
  if (process.env.NODE_ENV === 'production') {
    if (!pem || !pem.includes('-----BEGIN')) {
      throw new Error(
        '[SECURITY] FATAL: JWT_PRIVATE_KEY must be set in production with a valid PEM (RS256). ' +
        'Generate with: openssl genrsa 2048 -out private.pem'
      );
    }
    return pem;
  }
...
const JWT_REFRESH_EXPIRES_IN = process.env.JWT_REFRESH_EXPIRES_IN || jwtConfig.refreshExpiresIn;
...
/**
 * Generate JWT access token (RS256 or HS256 mock for development)
 */
export function generateAccessToken(user: User): string {
  ...
  // Use mock HS256 tokens in development when ALLOW_MOCK_AUTH is set
  if (!JWT_PRIVATE_KEY && process.env.ALLOW_MOCK_AUTH === 'true') {
    console.log('[AUTH] Using mock JWT token (HS256) - development only');
  ...
  if (!JWT_PRIVATE_KEY) {
    throw new Error('JWT_PRIVATE_KEY not configured. Set keys for JWT auth or use ALLOW_MOCK_AUTH=true in development.');
  }
...
/**
 * Verify JWT access token (RS256 or HS256 mock for development)
 */
export function verifyAccessToken(token: string): JWTPayload | null {
  try {
    let decoded: any;
    
    // Try mock HS256 tokens in development when ALLOW_MOCK_AUTH is set
    if (!JWT_PUBLIC_KEY && process.env.ALLOW_MOCK_AUTH === 'true') {
      decoded = jwt.verify(token, 'development-secret-key-not-for-production', {
        algorithms: ['HS256'],
        issuer: jwtConfig.issuer,
        audience: jwtConfig.audience
      });
    } else if (JWT_PUBLIC_KEY) {
  ...
  if (!userData && pool) {
    try {
      const result = await pool.query(
        `SELECT id, email, password_hash as "passwordHash", first_name as "firstName", last_name as "lastName", created_at, updated_at 
      ...
      if (result.rows.length > 0) {
        ...
        const user: User = {
          ...
          displayName: row.firstName && row.lastName
            ...
            : row.firstName || normalizedEmail.split('@')[0],
          role: assignedRole,
          createdAt: row.created_at,
          updatedAt: row.updated_at
        };
        // Add to memory store for future requests
        userStore.set(normalizedEmail, {
          user,
          passwordHash: row.passwordHash || '',
          refreshTokens: new Set()
        });
        userData = userStore.get(normalizedEmail);
        console.log(`[AUTH] User loaded from database: ${normalizedEmail}`);
      }
    } catch (error) {
      console.error(`[AUTH] Failed to fetch user from database:`, error);
    }
  }
