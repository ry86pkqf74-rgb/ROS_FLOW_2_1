    ...
    (set, get) => ({
      ...
      fetchProjects: async () => {
        ...
        try {
          ...
          if (result.error) throw new Error(result.error.error || 'Failed to fetch projects');
      ...
      fetchProjectById: async (id: string) => {
        ...
        try {
          ...
          const result = await api.get<{ project: ProjectWithWorkflows }>(`/api/projects/${id}`);
          if (result.error) throw new Error(result.error.error || 'Failed to fetch project');
          set({ currentProject: result.data?.project || null, isLoading: false });
        } catch (error) {
          set({ error: (error as Error).message, isLoading: false });
        }
      },
      ...
      createProject: async (data: CreateProjectInput) => {
        ...
        try {
          ...
          if (result.error) throw new Error(result.error.error || 'Failed to create project');
      ...
      updateProject: async (id: string, data: UpdateProjectInput) => {
        ...
        try {
          ...
          if (result.error) throw new Error(result.error.error || 'Failed to update project');
      ...
      deleteProject: async (id: string) => {
        ...
        try {
          ...
          if (result.error) throw new Error(result.error.error || 'Failed to delete project');
          set((state) => ({
            projects: state.projects.filter((p) => p.id !== id),
            currentProject: state.currentProject?.id === id ? null : state.currentProject,
          }));
      ...
      createWorkflow: async (projectId: string, data: CreateWorkflowInput) => {
        ...
        try {
          ...
          const result = await api.post<Workflow>(`/api/projects/${projectId}/workflows`, data);
          if (result.error) throw new Error(result.error.error || 'Failed to create workflow');
      ...
      updateWorkflow: async (projectId: string, workflowId: string, data: UpdateWorkflowInput) => {
        ...
        try {
          ...
          const result = await api.patch<Workflow>(`/api/projects/${projectId}/workflows/${workflowId}`, data);
          if (result.error) throw new Error(result.error.error || 'Failed to update workflow');
          const updated = result.data!;
          set((state) => ({
            currentProject: state.currentProject
              ? {
      ...
      deleteWorkflow: async (projectId: string, workflowId: string) => {
        ...
        try {
          ...
          const result = await api.delete(`/api/projects/${projectId}/workflows/${workflowId}`);
          if (result.error) throw new Error(result.error.error || 'Failed to delete workflow');
          set((state) => ({
            currentProject: state.currentProject
              ? {
                  ...state.currentProject,
      ...
      fetchStats: async () => {
        try {
          ...
          const result = await api.get<ProjectStats>('/api/projects/stats');
          if (result.error) {
      ...
      fetchRecentActivity: async () => {
        try {
          ...
          const result = await api.get<RecentActivity[]>('/api/projects/activity');
          if (result.error) {
