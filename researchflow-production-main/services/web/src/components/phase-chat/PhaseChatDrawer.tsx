...
export function PhaseChatDrawer({
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      const targetAgentId = selectedTopic || agents[0]?.id;
      const response = targetAgentId
        ? await sendAgentMessage(currentStage, targetAgentId, input)
        : await sendPhaseChatMessage(currentStage, input);
        ...
        {messages.map((msg) => (
          ...
          >
            ...
            {msg.role === 'assistant' &&
              ...
              Array.isArray(msg.metadata.ragContext) &&
              msg.metadata.ragContext.length > 0 && (
                <div className="mt-2 text-xs text-gray-500">
                  <div className="font-semibold">Context used</div>
                  <ul className="list-disc pl-4">
                    {msg.metadata.ragContext.slice(0, 3).map((ctx: any, idx: number) => (
                      <li key={`${msg.id}-ctx-${idx}`}>{ctx.content}</li>
                    ))}
                  </ul>
                </div>
              )}
