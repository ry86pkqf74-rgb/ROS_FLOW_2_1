...
import { ExportPanel } from "@/components/manuscript";
...
interface ManuscriptApiResponse {
  id: string;
  title?: string;
  status?: string;
...
interface ManuscriptSectionsResponse {
  sections: Array<{
    type: string;
    content: string;
    wordCount: number;
...
function buildSectionsSnapshot(sections: ManuscriptSection[]) {
  return sections.reduce<Record<string, { content: string; wordCount: number; lastGenerated?: string }>>(
    (acc, section) => {
      acc[section.id] = {
        content: section.content,
        wordCount: section.wordCount,
        lastGenerated: section.lastGenerated,
      };
      return acc;
    },
    {}
...
function buildContentText(sections: ManuscriptSection[]): string {
  return sections
...
function resolveTargetJournal(manuscript: ManuscriptApiResponse | null): string {
  if (!manuscript) return "thyroid";
  return manuscript.targetJournal || manuscript.target_journal || "thyroid";
}
...
export default function ManuscriptEditorPage() {
  ...
  const { data: manuscriptPayload, isLoading } = useQuery({
    queryKey: ["manuscript", manuscriptId],
    queryFn: async () => {
      if (manuscriptId === "new") return null;
      const [metaRes, sectionsRes] = await Promise.all([
        apiRequest("GET", `/api/manuscripts/${manuscriptId}`),
        apiRequest("GET", `/api/manuscripts/${manuscriptId}/sections`),
      ]);
      const meta = (await metaRes.json()) as ManuscriptApiResponse;
      const sections = (await sectionsRes.json()) as ManuscriptSectionsResponse;
      return { meta, sections: sections.sections || [] };
    },
  ...
  useEffect(() => {
    ...
    const { meta, sections: apiSections } = manuscriptPayload;
    setTitle(meta.title || "Untitled Manuscript");
    setTargetJournal(resolveTargetJournal(meta));

    const fallbackSections = meta.content?.sections
      ? Object.entries(meta.content.sections).map(([type, data]: [string, any]) => ({
          type,
    ...
    const resolvedSections = apiSections.length > 0 ? apiSections : fallbackSections;

    if (resolvedSections.length > 0) {
      setSections(
        SECTION_TEMPLATES.map((template) => {
          const saved = resolvedSections.find((s) => s.type === template.id);
          const content = saved?.content || "";
          const wordCount = saved?.wordCount ?? countWords(content);
          return {
            ...template,
            content,
            wordCount,
  ...
  const generateSection = useMutation({
    mutationFn: async (sectionId: string) => {
      ...
      if (manuscriptId === "new" && (sectionId === "results" || sectionId === "discussion")) {
        throw new Error("Save the manuscript before generating this section.");
      }

      let endpoint = "";
      const resolvedManuscriptId = manuscriptId === "new" ? undefined : manuscriptId;
      let payload: any = resolvedManuscriptId ? { manuscriptId: resolvedManuscriptId } : {};
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const sectionsSnapshot = buildSectionsSnapshot(sections);
      const contentText = buildContentText(sections);
      let activeId = manuscriptId;
      ...
      if (manuscriptId === "new") {
        ...
        const res = await apiRequest("PATCH", `/api/manuscripts/${manuscriptId}`, {
          title,
          targetJournal,
        });
        await res.json();
      ...
      const saveRes = await apiRequest("POST", `/api/manuscripts/${activeId}/doc/save`, {
        sections: sectionsSnapshot,
        contentText,
        changeDescription: manuscriptId === "new" ? "Initial draft" : "Draft update",
      });
      await saveRes.json();

      return { id: activeId, created: manuscriptId === "new" };
    },
    onSuccess: (data) => {
      toast({
      ...
      queryClient.invalidateQueries({ queryKey: ["manuscript", data.id] });
