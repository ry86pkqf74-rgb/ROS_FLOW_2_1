...
class MercuryProvider:
    ...
    - ``MERCURY_API_KEY`` (required): bearer token used for authentication.
      If it is not set, this provider falls back to ``INCEPTION_API_KEY`` or
      ``INCEPTIONLABS_API_KEY``. When no key is set, this provider fails closed
    ...
    def generate_text(
        ...
        *,
        ...
        if not api_key:
            raise RuntimeError(
                "Mercury API key not set (MERCURY_API_KEY, INCEPTION_API_KEY, or INCEPTIONLABS_API_KEY)."
            )

        base_url = os.getenv(
            "MERCURY_BASE_URL",
            os.getenv("INCEPTION_API_BASE_URL", "https://api.inceptionlabs.ai/v1"),
        )
