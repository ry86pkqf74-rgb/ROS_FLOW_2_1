...
Phase 3: per-stage structlog and Prometheus metrics.
"""
...
from src.utils.logging import get_logger, log_stage_start, log_stage_end
from src.utils.metrics import record_stage
...
logger = get_logger("workflow_engine.runner")
...
async def run_stages(
    ...
    for stage_id in stage_ids:
        ...
        if stages_failed and stop_on_failure:
            ...
            logger.info("stage_skipped_previous_failure", stage_id=stage_id)
            continue

        # Get the registered stage
        stage_cls = get_stage(stage_id)
        if stage_cls is None:
            logger.warning("stage_not_registered", stage_id=stage_id)
            stages_skipped.append(stage_id)
            continue
        ...
        log_stage_start(
            logger, stage_id, stage.stage_name, context.job_id,
            project_id=context.metadata.get("project_id") if context.metadata else None,
        )
        ...
        try:
            ...
            if result.status == "completed":
                ...
                stages_skipped.append(stage_id)
                logger.info("stage_skipped", stage_id=stage_id)

        except Exception as e:
            logger.exception("stage_exception", stage_id=stage_id, error=str(e))
            ...
            results[stage_id] = result
            stages_failed.append(stage_id)
            record_stage(stage_id, "failed", result.duration_ms / 1000.0)
            log_stage_end(
                logger,
                stage_id,
                stage.stage_name,
