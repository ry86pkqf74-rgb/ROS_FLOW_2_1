...
# FastAPI app
app = FastAPI(
    title="Clinical Section Drafter Agent Proxy",
    description="LangSmith cloud proxy for agent-clinical-section-drafter",
    version="1.0.0",
    lifespan=lifespan
)
...
async def health():
    ...
    return {"status": "ok", "service": "agent-section-drafter-proxy"}
...
async def run_agent_sync(request: AgentRunRequest):
    ...
    Proxies to LangSmith cloud agent for Clinical Section Drafter
    """
    ...
    langsmith_payload = {
        ...
        "input": {
            "section_type": request.inputs.get("section_type", "Results"),
            "study_summary": request.inputs.get("study_summary", ""),
            "results_data": request.inputs.get("results_data", {}),
            "evidence_chunks": request.inputs.get("evidence_chunks", []),
            "key_hypotheses": request.inputs.get("key_hypotheses", []),
    ...
    try:
        ...
        return AgentRunResponse(
            ...
            request_id=request.request_id,
            outputs={
                "section_draft": output.get("section_draft", ""),
                "guideline_compliance_report": output.get("guideline_compliance_report", {}),
                "evidence_citations": output.get("evidence_citations", []),
