"""
agent-results-writer: write Results section from outline, verifiedClaims, extractionRows, groundingPack.
...
async def _execute_section_write(inputs: Dict[str, Any], mode: str, request_id: str) -> Dict[str, Any]:
    ...
    metadata = {
        "agentId": "agent-results-writer",
        "projectId": "researchflow",
        "runId": request_id,
        "threadId": "thread-results-writer",
        "stageRange": [1, 20],
        "currentStage": 14,
    }
    ...
    try:
        ...
        logger.warning("results_writer_bridge_failed", error=str(type(e).__name__))
        return {
            "section_markdown": "",
            "sectionMarkdown": "",
