"""POST /agents/run/sync and /agents/run/stream; contract envelope and RAG_RETRIEVE task type."""
import time
import orjson
...
from agent.impl import TASK_TYPE, run_sync, run_stream
...
def _reject_task_type(req: AgentRunRequest) -> JSONResponse | None:
    ...
    body = {
        ...
        "error": {
            ...
            "message": f"task_type must be {TASK_TYPE}",
...
async def agents_run_sync(req: AgentRunRequest):
    reject = _reject_task_type(req)
    if reject is not None:
        return reject
    started = time.time()
    logger.info("sync_request", request_id=req.request_id, task_type=req.task_type)
    try:
...
async def agents_run_stream(req: Request):
    ...
    logger.info("stream_request_start", request_id=request_id, task_type=task_type)

    async def event_generator():
        async for evt in run_stream(payload):
            yield {
                "event": evt.get("type", "message"),
                "data": orjson.dumps(evt, default=str).decode("utf-8"),
            }
