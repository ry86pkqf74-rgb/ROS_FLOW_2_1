...
# FastAPI app
app = FastAPI(
    title="Journal Guidelines Cache Agent Proxy",
    description="LangSmith cloud proxy for agent-journal-guidelines-cache",
    version="1.0.0",
    lifespan=lifespan
)
...
async def health():
    ...
    return {"status": "ok", "service": "agent-journal-guidelines-cache-proxy"}
...
async def run_agent_sync(request: AgentRunRequest):
    ...
    Proxies to LangSmith cloud agent for Journal Guidelines Cache
    """
    ...
    langsmith_payload = {
        ...
        "input": {
            "action": request.inputs.get("action", "get_guidelines"),
            "journal_name": request.inputs.get("journal_name"),
            "journal_names": request.inputs.get("journal_names", []),
            "force_refresh": request.inputs.get("force_refresh", False),
            "spreadsheet_id": request.inputs.get("spreadsheet_id"),
            "request_id": request.request_id,
            "mode": request.mode,
        },
        "config": {
    ...
    try:
        ...
        outputs = {
            "guidelines": langsmith_response.get("output", {}).get("guidelines", ""),
            "cache_status": langsmith_response.get("output", {}).get("cache_status", ""),
            "last_updated": langsmith_response.get("output", {}).get("last_updated"),
            "aliases": langsmith_response.get("output", {}).get("aliases", []),
            "source_urls": langsmith_response.get("output", {}).get("source_urls", []),
            "changelog": langsmith_response.get("output", {}).get("changelog"),
            "comparison_table": langsmith_response.get("output", {}).get("comparison_table"),
            "cache_stats": langsmith_response.get("output", {}).get("cache_stats"),
