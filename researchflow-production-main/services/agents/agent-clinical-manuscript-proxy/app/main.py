...
# FastAPI app
app = FastAPI(
    title="Clinical Manuscript Writer Agent Proxy",
    description="LangSmith cloud proxy for agent-clinical-manuscript",
    version="1.0.0",
    lifespan=lifespan
)
...
async def health():
    ...
    return {"status": "ok", "service": "agent-clinical-manuscript-proxy"}
...
async def run_agent_sync(request: AgentRunRequest):
    ...
    Proxies to LangSmith cloud agent for Clinical Manuscript Writer
    """
    ...
    langsmith_payload = {
        ...
        "input": {
            "study_data": request.inputs.get("study_data", {}),
            "evidence_log": request.inputs.get("evidence_log", []),
            "protocol_data": request.inputs.get("protocol_data", {}),
            "request_id": request.request_id,
            "mode": request.mode,
        },
        "config": {
    ...
    try:
        ...
        return AgentRunResponse(
            ...
            request_id=request.request_id,
            outputs={
                "manuscript_draft": output.get("manuscript_draft", ""),
                "evidence_ledger_url": output.get("evidence_ledger_url"),
                "google_doc_url": output.get("google_doc_url"),
                "compliance_report": output.get("compliance_report", {}),
                "statistical_review": output.get("statistical_review", {}),
