...
# FastAPI app
app = FastAPI(
    title="Artifact Auditor Agent Proxy",
    description="LangSmith cloud proxy for agent-artifact-auditor",
    version="1.0.0",
    lifespan=lifespan
)
...
async def health():
    ...
    return {"status": "ok", "service": "agent-artifact-auditor-proxy"}
...
async def run_agent_sync(request: AgentRunRequest):
    ...
    Proxies to LangSmith cloud agent for Artifact Auditor
    """
    ...
    langsmith_payload = {
        ...
        "input": {
            "artifact_source": request.inputs.get("artifact_source", ""),
            "artifact_location": request.inputs.get("artifact_location", ""),
            "reporting_standard": request.inputs.get("reporting_standard", ""),
            "standard_version": request.inputs.get("standard_version"),
            "custom_guidelines": request.inputs.get("custom_guidelines"),
            "github_repository": request.inputs.get("github_repository"),
            "github_file_path": request.inputs.get("github_file_path"),
            "google_doc_id": request.inputs.get("google_doc_id"),
            "artifact_content": request.inputs.get("artifact_content"),
            "request_id": request.request_id,
            "mode": request.mode,
        },
        "config": {
    ...
    try:
        ...
        outputs = {
            "audit_summary": langsmith_response.get("output", {}).get("audit_summary", {}),
            "compliance_score": langsmith_response.get("output", {}).get("compliance_score"),
            "critical_issues": langsmith_response.get("output", {}).get("critical_issues", []),
            "major_issues": langsmith_response.get("output", {}).get("major_issues", []),
            "minor_issues": langsmith_response.get("output", {}).get("minor_issues", []),
            "equity_flags": langsmith_response.get("output", {}).get("equity_flags", []),
            "audit_report_url": langsmith_response.get("output", {}).get("audit_report_url"),
            "audit_log_entry": langsmith_response.get("output", {}).get("audit_log_entry", {}),
        ...
        return AgentRunResponse(
            ok=False,
            request_id=request.request_id,
            error="LangSmith API timeout - audit took too long"
        )
