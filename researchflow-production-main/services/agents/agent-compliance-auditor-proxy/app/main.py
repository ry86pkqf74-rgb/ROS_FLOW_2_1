...
# FastAPI app
app = FastAPI(
    title="Compliance Auditor Agent Proxy",
    description="LangSmith cloud proxy for agent-compliance-auditor",
    version="1.0.0",
    lifespan=lifespan
)
...
async def health():
    ...
    return {"status": "ok", "service": "agent-compliance-auditor-proxy"}
...
async def run_agent_sync(request: AgentRunRequest):
    ...
    Proxies to LangSmith cloud agent for Compliance Auditor
    """
    ...
    langsmith_payload = {
        ...
        "input": {
            "log_source": request.inputs.get("log_source", ""),
            "log_data": request.inputs.get("log_data", ""),
            "frameworks": request.inputs.get("frameworks", ["HIPAA", "GDPR", "EU AI Act"]),
            "include_code_scan": request.inputs.get("include_code_scan", False),
            "repository": request.inputs.get("repository"),
            "tracker_spreadsheet_id": request.inputs.get("tracker_spreadsheet_id"),
            "request_id": request.request_id,
            "mode": request.mode,
        },
        "config": {
    ...
    try:
        ...
        outputs = {
            "executive_summary": langsmith_response.get("output", {}).get("executive_summary", {}),
            "scan_results": langsmith_response.get("output", {}).get("scan_results", []),
            "audit_findings": langsmith_response.get("output", {}).get("audit_findings", []),
            "remediation_plan": langsmith_response.get("output", {}).get("remediation_plan", []),
            "remediation_tracker_status": langsmith_response.get("output", {}).get("remediation_tracker_status"),
            "regulatory_updates": langsmith_response.get("output", {}).get("regulatory_updates"),
            "audit_trail_metadata": langsmith_response.get("output", {}).get("audit_trail_metadata", {}),
            "generated_artifacts": langsmith_response.get("output", {}).get("generated_artifacts", {}),
