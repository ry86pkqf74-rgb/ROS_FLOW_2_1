...
# FastAPI app
app = FastAPI(
    title="Results Interpretation Agent Proxy",
    description="LangSmith cloud proxy for agent-results-interpretation",
    version="1.0.0",
    lifespan=lifespan
)
...
async def health():
    ...
    return {"status": "ok", "service": "agent-results-interpretation-proxy"}
...
async def run_agent_sync(request: AgentRunRequest):
    ...
    langsmith_payload = {
        ...
        "input": {
            ...
            "study_metadata": request.inputs.get("study_metadata", {}),
            "request_id": request.request_id,
            "mode": request.mode,
        },
        "config": {
