"""
Unit tests for Stage 18: ImpactAssessmentAgent

Tests the ImpactAssessmentAgent implementation:
- Initialization and stage identity
- get_tools (empty) and get_prompt_template
- execute with mocked bridge (runStageImpactAssessment)
...
from src.workflow_engine.stages.stage_18_impact import ImpactAssessmentAgent
from src.workflow_engine.types import StageContext


@pytest.fixture
def agent():
    """Create an ImpactAssessmentAgent instance."""
    return ImpactAssessmentAgent({"bridge_url": "http://localhost:3001"})
...
class TestImpactAssessmentAgent:
    """Tests for ImpactAssessmentAgent class."""

    def test_agent_initialization(self, agent):
        """Agent should initialize with correct stage ID and name."""
        assert agent.stage_id == 18
        assert agent.stage_name == "Impact Assessment"

    def test_agent_initialization_without_config(self):
        """Agent should initialize without config (bridge_config None)."""
        a = ImpactAssessmentAgent()
        assert a.stage_id == 18
        assert a.stage_name == "Impact Assessment"
...
class TestExecute:
    ...
    async def test_execute_success(self, agent, sample_context):
        ...
        mock_response = {"ok": True, "impact_metrics": {}}
        with patch.object(
