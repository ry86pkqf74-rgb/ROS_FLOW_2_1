"""
Unit tests for Stage 19: DisseminationAgent

Tests the DisseminationAgent implementation:
- Initialization and stage identity
- get_tools (empty) and get_prompt_template
- execute with mocked bridge (runStageDissemination)
...
from src.workflow_engine.stages.stage_19_dissemination import DisseminationAgent
from src.workflow_engine.types import StageContext


@pytest.fixture
def agent():
    """Create a DisseminationAgent instance."""
    return DisseminationAgent({"bridge_url": "http://localhost:3001"})
...
def sample_context(tmp_path):
    ...
    return StageContext(
        job_id="test-job",
        config={"target_venue": "journal"},
        artifact_path=str(tmp_path / "artifacts"),
        governance_mode="DEMO",
    )


class TestDisseminationAgent:
    """Tests for DisseminationAgent class."""

    def test_agent_initialization(self, agent):
        """Agent should initialize with correct stage ID and name."""
        assert agent.stage_id == 19
        assert agent.stage_name == "Dissemination"

    def test_agent_initialization_without_config(self):
        """Agent should initialize without config (bridge_config None)."""
        a = DisseminationAgent()
        assert a.stage_id == 19
        assert a.stage_name == "Dissemination"
...
class TestExecute:
    ...
    async def test_execute_success(self, agent, sample_context):
        ...
        mock_response = {"ok": True, "export_bundle": []}
        with patch.object(
