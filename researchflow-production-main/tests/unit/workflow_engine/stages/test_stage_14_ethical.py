"""
Unit tests for Stage 14: EthicalReviewAgent

Tests the EthicalReviewAgent implementation:
- Initialization and stage identity
- get_tools (empty) and get_prompt_template
- execute with mocked bridge (runStageEthicalReview)
...
from src.workflow_engine.stages.stage_14_ethical import EthicalReviewAgent
from src.workflow_engine.types import StageContext


@pytest.fixture
def agent():
    """Create an EthicalReviewAgent instance."""
    return EthicalReviewAgent({"bridge_url": "http://localhost:3001"})
...
def sample_context(tmp_path):
    ...
    return StageContext(
        job_id="test-job",
        config={"compliance_checklist": []},
        artifact_path=str(tmp_path / "artifacts"),
        governance_mode="DEMO",
    )


class TestEthicalReviewAgent:
    """Tests for EthicalReviewAgent class."""

    def test_agent_initialization(self, agent):
        """Agent should initialize with correct stage ID and name."""
        assert agent.stage_id == 14
        assert agent.stage_name == "Ethical Review"

    def test_agent_initialization_without_config(self):
        """Agent should initialize without config (bridge_config None)."""
        a = EthicalReviewAgent()
        assert a.stage_id == 14
        assert a.stage_name == "Ethical Review"
...
class TestExecute:
    ...
    async def test_execute_success(self, agent, sample_context):
        ...
        mock_response = {"ok": True, "compliance_checklist": []}
        with patch.object(
...
class TestErrorHandling:
    ...
    async def test_execute_completion_on_bridge_failure(self, agent, sample_context):
        """Bridge failure should not complete; status failed and warnings/errors present."""
        with patch.object(
            agent, "call_manuscript_service", new_callable=AsyncMock
        ) as mock_svc:
            mock_svc.side_effect = Exception("Bridge unavailable")
