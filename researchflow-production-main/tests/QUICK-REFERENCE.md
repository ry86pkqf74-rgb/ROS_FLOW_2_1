...
| Docker E2E | `npm run test:e2e:docker` | ~5m | Requires Docker stack up |

## Docker deployment E2E (Live mode)

Runs Playwright against the full Docker stack to verify buttons, cards, LIVE mode, manuscript drafting, and chat/RAG after login.

**Prerequisites:** Docker Compose stack running (`docker compose up -d`).

```bash
# Start stack then run tests (or use --no-up if already up)
./scripts/run-docker-e2e.sh

# Or manually: ensure stack is up, then
npm run test:e2e:docker

# With custom URLs
PLAYWRIGHT_BASE_URL=http://localhost:5173 API_URL=http://localhost:3001 \
  npx playwright test tests/e2e/docker-deployment-live.spec.ts
```

**Optional:** Set `ENABLE_DEV_AUTH=true` on the orchestrator service to run dev-auth LIVE flow tests.
